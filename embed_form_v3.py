...
js_48_replacements = r'''            currentPoNo = String(data[5] || "");
            ...
            // Updated data[15] to data[14] to match Item Description column
            if (!data[2] && !data[3] && !data[14]) stateStr = "Initiated Order";
            ...
            let ctx = {
                ...
                poNo: currentPoNo,
                supplier: formatIfDate(data[11]),
                blNo: formatIfDate(data[28]),
                piNo: formatIfDate(data[12]),
                delTerm: formatIfDate(data[25]),
                ...
                rawObj: {
                    ...
                    poIssDate: formatIfDate(data[6]),
                    poApprDate: formatIfDate(data[7]),
                    reqDelDate: formatIfDate(data[8]),
                    company: formatIfDate(data[9]),
                    supplier: formatIfDate(data[11]),
                    piNo: formatIfDate(data[12]),
                    desc: formatIfDate(data[14]),
                    qty: formatIfDate(data[15]),
                    fob: formatIfDate(data[18]),
                    cif: formatIfDate(data[19]),
                    value: formatIfDate(data[20]),
                    currency: formatIfDate(data[21]),
                    payTerm: formatIfDate(data[23]),
                    acid: formatIfDate(data[24]),
                    delTerm: formatIfDate(data[25]),
                    commInv: formatIfDate(data[27]),
                    blNo: formatIfDate(data[28]),
                    etd: formatIfDate(data[29]),
                    eta: formatIfDate(data[30]),
                    shipRelease: formatIfDate(data[33]),
                    containers: formatIfDate(data[34]),
                    containerType: formatIfDate(data[35]),
                    bank: formatIfDate(data[36]),
                    bankDate: formatIfDate(data[37]),
                    estReadiness: formatIfDate(data[38]),
                    leadTime: formatIfDate(data[40]),
                    vendorEmail: formatIfDate(data[42]),
                    buyerEmail: formatIfDate(data[43]) || localStorage.getItem('ow_current_user') || "",
